...
function JWTAuthentication() {
    ...
    const userIsAdmin = async (token) => {
        return ((await jsonwebtoken_1.default.verify(token, TOKEN_SECRET).user_type) === AccountType.Admin);
    };
    const userIsManager = async (token) => {
        return ((await jsonwebtoken_1.default.verify(token, TOKEN_SECRET).user_type) === AccountType.Manager);
    };
    const userIsUser = async (token) => {
        return ((await jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET).user_type) ===
            AccountType.User);
    };
    return {
        verifyJWTToken,
